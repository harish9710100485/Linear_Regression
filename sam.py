...
# Specify the input file path (update this with your actual file path)
INPUT_FILE_PATH = r"C:\Users\haris\Desktop\Day14\original_Sales_Data_for_Analysis.tsv"
...
def process_dataframe(df):
    try:
        # Ensure necessary columns exist, otherwise create them with default values
        required_columns = ["PART NO", "QTY", "PERIOD", "UNIT PRICE", "CURRENCY", "EX RATE", "TOTAL PRICE (INR)"]
        ...
        df["year"] = df["year"].ffill().bfill()  # Fill missing years
        df["year"] = df["year"].astype(int)

        df["Currency"] = df["Currency"].fillna("INR").str.strip().str.upper()
        df["Exchange Rate"] = df["Exchange Rate"].fillna(75)  # Default exchange rate

        # Convert USD & EUR to INR
        df.loc[df["Currency"] == "USD", "Item Total"] *= df["Exchange Rate"]
        df.loc[df["Currency"] == "EUR", "Item Total"] *= df["Exchange Rate"]
        df["Currency"] = "INR"  # Convert everything to INR

        # Handle missing numeric values
        df["Quantity"] = df["Quantity"].fillna(df["Quantity"].mean())
        df["Item Total"] = df["Item Total"].fillna(df["Item Total"].mean())
...
def main():
    ...
    try:
        ...
        df = pd.read_csv(INPUT_FILE_PATH, sep=delimiter, engine='python', on_bad_lines='skip')
    except Exception as e:
